...
![](/assets/Snip20180711_13.png)<br>
  ...
  <br>**clear 属性注意点**<br> 当给某个标签(盒子)设置clear 属性后,再给它设置 margin 属性时就会失效,主要原因是:<br>(1) 如果给子元素设置margin-top 父元素如果没有边框,那么父元素会被顶下来.
    ...
    **注意点:**<br> 外墙法只能使第二个何止设置margin-top 不能使第一个盒子设置 margin-bottom. 因此我们在使用外墙法时为了保重中间有个间距通常的做法不设置第一个盒子的margin-bottom 也不设置 第二个盒子的margin-top, 而是设置中间盒子的 height
    ...
    <div class="wall"></div>
    ```
    (2)内墙法:<br> 2.1 在第一个盒子中的所有子元素的最后添加一个额外的块级元素<br>2.2 给这个额外添加的块级元素设置 clear : both.<br><br> **注意点:**(1)内墙法可以让第一个盒子设置margin-bottom 属性<br>(2)内墙法可以让第二个盒子设置margin-top 属性<br>(3) 内墙法也可以设置内墙的高度.<br><br> **外墙法和内墙法的区别:**<br> 外墙法不能撑起第一个盒子的高度,而内墙法可以撑起第一个盒子的高度<br><br> 在企业开发中,我们很少使用隔墙法内墙法和外墙法来清除浮动
