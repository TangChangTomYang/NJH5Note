...
- 1 **什么是网页的布局方式?**<br>
 网页的布局方式其实就是指浏览器是如何对网页中的元素进行排版的.**主要有三中方式:**<br> 
 (1)标准流 (文档流\普通流) 排版方式.
 (2)浮动流排版方式. 
 (3)定位流排版方式.
...
- **标准流排版**
    ...
    - 在标准流中有2种排版, **垂直排版 \ 水平排版**<br> 
    &emsp; **块级元素(div h p)垂直排版**<br>
    &emsp; **行内(span b strong)\行内块级(img)水平排版**
    - 在标准流中,**默认情况下 块级元素和父标签一样宽度,默认情况下块级元素没有高度.**
    - 在默认情况下, 内容的高度是可以撑起盒子的高度的(块级标签的高度),**即默认情况下 块级标签和内容的高度一样(块的高度由内容撑起来).**
...
- **浮动流排版**
    ...
    **<br><br>  浮动流注意:**
(1)浮动流中没有居中对齐, **只有 none left right 3个取值**<br>
(2)在浮动流中是 **不能使用 margin:0px auto; 这种方式来让子元素在父元素中水平居中的**
    **<br> 浮动流特点:**
    (1) 在浮动流中是 **不区分 块级元素 \ 行内元素 \ 行内块级元素的, **都可以水平排版的
    (2) 在浮动流无论是 块级元素 \ 行内元素 \ 行内块级元素 **都可以设置 宽高**.
    (3) 综上所述, 浮动流中的元素和标准流中的 **行内块级元素很像.**,既可以水平排版又可以设置宽高
    ![](/assets/Snip20180716_9.png)
    
    - 什么浮动元素脱标?
    脱标: 即脱离标准流, **当某个(块级)元素浮动后,那么这个元素就像从标准流中删除了一样, 这就是浮动元素脱标.**
 ...
 - 浮动元素的字围现象**(主要应用场景是图文混排)**<br> 什么是浮动元素的字围现象,即 浮动元素不会盖住 不会浮动元素的内容,即如下图:<br>![](/assets/Snip20180711_11.png)
    ...
    - **浮动元素高度问题**
        ...
        <br><br>
        **清除浮动的方式1:**
        给前面一个父标签(块级标签)设置高(高度需要大于等于最低的一个子标签),这样的话后一个盒子的浮动元素就不会去找前一个盒子的浮动元素(因为不是同一行了) <br>
